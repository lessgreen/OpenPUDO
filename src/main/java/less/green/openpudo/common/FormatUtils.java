...
public class FormatUtils {
    ...
    public static String smartSize(long bytes) {
        if (bytes < 1_024L) {
            ...
            return BigDecimal.valueOf(bytes).divide(BigDecimal.valueOf(1_024L), bytes < 10L * 1_024L ? 2 : 1, RoundingMode.HALF_UP) + " KiB";
        } else if (bytes < 1_073_741_824L) {
            return BigDecimal.valueOf(bytes).divide(BigDecimal.valueOf(1_048_576L), bytes < 10L * 1_048_576L ? 2 : 1, RoundingMode.HALF_UP) + " MiB";
        } else if (bytes < 1_099_511_627_776L) {
            return BigDecimal.valueOf(bytes).divide(BigDecimal.valueOf(1_073_741_824L), bytes < 10L * 1_073_741_824L ? 2 : 1, RoundingMode.HALF_UP) + " GiB";
        } else if (bytes < 1_125_899_906_842_624L) {
            return BigDecimal.valueOf(bytes).divide(BigDecimal.valueOf(1_099_511_627_776L), bytes < 10L * 1_099_511_627_776L ? 2 : 1, RoundingMode.HALF_UP) + " TiB";
        } else {
            return BigDecimal.valueOf(bytes).divide(BigDecimal.valueOf(1_125_899_906_842_624L), bytes < 10L * 1_125_899_906_842_624L ? 2 : 1, RoundingMode.HALF_UP) + " PiB";
        }
    }
